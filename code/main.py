...
def main():
    ...
    with torch.no_grad(): 
      out = model.forward(feature_dict, edge_dict, feature_dict_refined, edge_dict_refined)
    ...
    refined_graph['user'].features = embeddings.cpu()
    ...
    with torch.no_grad():
        ...
        if args.return_embeddings:
            embeddings = model(node_features, edge_index, return_embedding=True)
            embeddings = embeddings[test_indices].cpu().numpy()
            print('saving...')
            np.save('./embeddings/habit_embeddings.npy', embeddings)
        ...
        nmi = normalized_mutual_info_score(test_truth, test_predictions)
        ari = adjusted_rand_score(test_truth, test_predictions)
